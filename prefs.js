...
export default class UsdRubPreferences extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        ...
        // Try to get settings via helper (available in GNOME 47+)
        let settings = null;
        try {
            settings = this.getSettings(SCHEMA);
        } catch (_) {}

        // Fallbacks: load schema from local directory, then by schema_id
        if (!settings) {
            try {
        ...
        const row = new Adw.ComboRow({ title: 'Indicator position', model });
